...
- **Feature parity audit** (updated 2026-07-08): The web frontend MUST strictly match the Flutter mobile app. **Web cannot have features that don't exist in Flutter.** If Flutter disabled a feature (e.g. `canDelete: false`), Web must also disable it. Key rules: (1) Flutter has no delete buttons in publish lists → Web removed them; (2) Flutter booking lists have no inline confirm/reject → Web removed them; (3) Flutter booking lists have no status filter tabs → Web removed them; (4) Flutter `onAddCity()` does VIP check → navigate to form, NOT direct API call; (5) API response format may differ: some endpoints return flat arrays (`data: [...]`) while others return wrapped (`data: {list: [...]}`) — always use `Array.isArray(data?.list) ? data.list : Array.isArray(data) ? data : []` pattern. Flutter reference at `/tmp/lumotrip_ref/lumotrip/lib/pages/`. See [[flutter-web-feature-parity]], [[guide-publish-city-alignment]], and [[mobile-frontend-reference]] in memory.
...
- **Guide feature parity** (audited & fixed 2026-07-07, realigned 2026-07-08): Full Flutter-Web guide feature comparison. CRITICAL fix: created `publish-city-form.js` (city creation/editing form, 14 fields + 3-level cascade + 6-image upload). HIGH fixes: profile menu navigation (3 broken links), publish form expansion (category selectors, phone/email/website/tickets/images). **2026-07-08 realignment**: Removed Web-only features not in Flutter — publish list delete buttons (Flutter `canDelete: false`), booking list inline confirm/reject, booking status filter tabs, 「申請已有城市」panel (replaced with VIP gate → form navigation matching Flutter `onAddCity()`). Also fixed guideCityList API data format (flat array vs wrapped). See [[guide-feature-parity-2026-07-07]] and [[flutter-web-feature-parity]] in memory.
