/* ============================================
   Merchant Bookings — 商家预约管理 + 预约详情
   Reference: PPCC company-panel/bookings/page.tsx
   ============================================ */
  ...
  template: `
    <div class="page-content">
      ...
      <div v-else class="ds-container-640" style="padding-bottom:32px">
        ...
        <!-- List -->
        <div v-else>
          <div v-for="b in bookings" :key="b.id" @click="$router.push('/merchant/booking-detail/'+b.id)"
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px">
              <div>
                <span style="font-size:11px;color:var(--color-assistant-text)">{{ $t('商家') }}</span>
                <p style="font-size:13px;font-weight:600;margin:0">{{ b.content?.name || '—' }}</p>
              </div>
              <div>
                <span style="font-size:11px;color:var(--color-assistant-text)">{{ $t('到達時間') }}</span>
  ...
  methods: {
    async fetchBookings() {
      ...
      try {
        ...
        const result = await ApiProvider.get(ApiUrl.companyReserve, params);
        if (result.success) {
    ...
    statusInfo(s) { return MERCHANT_STATUS_MAP[Number(s)] || MERCHANT_STATUS_MAP[1]; },
    formatDate(d) {
      if (!d) return '—';
...
/*** MerchantBookingDetailPage ***/
const MerchantBookingDetailPage = {
  template: `
    <div class="page-content">
      <div v-if="!UserStore.isLogin" style="text-align:center;padding-top:80px">
        <div style="font-size:48px;margin-bottom:16px">📋</div>
      ...
      <div v-else class="ds-container-600" style="padding-bottom:32px">
        ...
        <div class="ds-card" style="padding:16px;margin-bottom:12px">
          ...
          <div class="ds-list-item" style="padding:8px 0">
            <span style="color:var(--color-assistant-text);font-size:13px">{{ $t('商家') }}</span>
            <span style="font-size:13px;font-weight:600">{{ detail.content?.name || '—' }}</span>
          </div>
          <div class="ds-list-item" style="padding:8px 0">
          ...
          <div class="ds-list-item" style="padding:8px 0">
            <span style="color:var(--color-assistant-text);font-size:13px">{{ $t('到達時間') }}</span>
            <span style="font-size:13px;font-weight:500">{{ formatDate(detail.arrival_time) }}</span>
          </div>
          <div v-if="detail.leave_time" class="ds-list-item" style="padding:8px 0">
            <span style="color:var(--color-assistant-text);font-size:13px">{{ $t('離開時間') }}</span>
            <span style="font-size:13px;font-weight:500">{{ formatDate(detail.leave_time) }}</span>
          </div>
          <div class="ds-list-item" style="padding:8px 0">
        ...
        <!-- Actions -->
        <div v-if="detail.status===1" style="display:flex;gap:12px;padding-bottom:32px">
  ...
  methods: {
    async fetchDetail() {
      ...
      try {
        const result = await ApiProvider.get(ApiUrl.companyReserveInfo, { id });
        if (result.success) {
    ...
    async handleConfirm() {
      ...
      try {
        const result = await ApiProvider.post(ApiUrl.companyConfirmReserve, { id: this.detail.id });
    ...
    async handleReject() {
      ...
      this.submitting = true;
      try {
        const result = await ApiProvider.post(ApiUrl.companyRejectReserve, { id: this.detail.id, reason });
        if (result.success) this.fetchDetail();
      } catch (e) { /* silent */ }
      this.submitting = false;
    },
