...
- **Guide feature parity** (audited & fixed 2026-07-07): Full Flutter-Web guide feature comparison. CRITICAL fix: created `publish-city-form.js` (city creation/editing form, 14 fields + 3-level cascade + 6-image upload). HIGH fixes: profile menu navigation (3 broken links), publish form expansion (category selectors, phone/email/website/tickets/images). Also added VIP gate, draft save/restore. Flutter reference extracted from `lumotrip(2).zip` to `/tmp/lumotrip_ref/`. See [[guide-feature-parity-2026-07-07]] in memory.
